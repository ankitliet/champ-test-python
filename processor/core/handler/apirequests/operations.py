...
from resource_adapters.providers.postprovisioning.operations import execute
...
class Operations:
    def __init__(self, session, task_id, config, env_vars):
        ...
        LOG.debug("APIRequests Operation init method", {'task_id': self.task_id})
    ...
    def __call__(self, operation_name, parameters: dict):
        LOG.debug("APIRequests Operation call:%s" % parameters, {'task_id': self.task_id})
        try:
            self.add_audit_log("APIRequests Operation", operation_name, "Started", TASK_STATUS.COMPLETED)
            parameters = execute(
                func_name=operation_name, parameters=parameters, 
                session=self.session, config=self.config, env_vars=self.env_vars
            )
            self.add_audit_log("APIRequests Operation", operation_name,
