        ...
        try:
            ...
            if suppress == False:
                if proc.returncode != 0:
                    raise TerraformFailedException(f'\nSTDOUT -> [{stdout_value.decode()}] \nSTDERR -> [{stderr_value.decode()}]')
            stdout_value = escape_ansi(stdout_value.decode())
            stderr_value = escape_ansi(stderr_value.decode())
            return proc.returncode, stdout_value, stderr_value
        except subprocess.TimeoutExpired as excp:
            proc.kill()
            stdout_value, stderr_value = proc.communicate()
            raise TerraformTimoutException(f'\nSTDOUT -> [{stdout_value.decode()}] \nSTDERR -> [{stderr_value.decode()}]') from excp
