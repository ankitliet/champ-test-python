...
class CommonRepository:
    ...
    def fetch(self, iden_filters: dict):
        with self.session_factory() as session:
            try:
                ...
                raise Exception(f'Table[{self._model._str()}] has no column[{excp.__str__().split("attribute ", 1)[1]}]')
            except sqexcp.NoResultFound as excp:
                raise Exception(f'No record for table[{self._model._str()}] with applied filter[{iden_filters}]were found!')
    ...
    def update(self, iden_filters: dict, **kwargs):
        with self.session_factory() as session:
            filters = [getattr(self._model, column)==value for column, value in iden_filters.items()]
            try:
                filter_query = session.query(self._model).filter(*filters)
