...
from resource_adapters.providers.vmware.dcoperations import *
...
class Operations:
    def __init__(self, session, task_id, config, env_vars):
        ...
        LOG.debug("Vsphere Operation init method", {'task_id': self.task_id})
    ...
    def __call__(self, operation_name, parameters: dict):
        LOG.debug("Vsphere Operation call:%s" % parameters, {'task_id': self.task_id})
        try:
            self.add_audit_log("Vsphere Operation", operation_name,
