...
from resource_adapters.providers.storage.smboperations import *
from resource_adapters.providers.storage.nfsoperations import *
...
class Operations:
    def __init__(self, session, task_id, config, env_vars):
        ...
        LOG.debug("Storage Operation init method", {'task_id': self.task_id})
    ...
    def __call__(self, operation_name, parameters: dict):
        LOG.debug("Storage Operation call:%s" % parameters, {'task_id': self.task_id})
        try:
            self.add_audit_log("Storage Operation", operation_name,
